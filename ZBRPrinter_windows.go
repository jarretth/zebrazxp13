...
func ZBRCloseHandle(handle Handle) (success SuccessReturn, err PrinterError) {
...
func ZBRGetHandle(drvName string) (success SuccessReturn, handle Handle, prn_type PrinterType, err PrinterError) {
...
func ZBRPRNEjectCard(handle Handle, prn_type PrinterType) (success SuccessReturn, err PrinterError) {
    ret, _, _ := zBRPRNEjectCard.Call(
            uintptr(handle),
            uintptr(uint(prn_type)),
            uintptr(unsafe.Pointer(&err)),
    )
    return SuccessReturn(ret), err
}
